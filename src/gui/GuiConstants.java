package gui;

import java.awt.Color;

public class GuiConstants {

	public static final int FRAME_HEIGHT = 750;
	public static final int FRAME_WIDTH = 1200;
	public static final int RESPONSE_SPACING = 2;	
	public static final int LINE_SPACING = 2;	
	public static final int RESPONSE_NAME_SPACING = 3;	
	
	
	public static final Color RESPONSE_COLOR = new Color(160, 50, 80, 170);
	public static final Color SLOT_COLOR = new Color(0, 200, 0, 170);
	public static final Color GRAY_OUT_COLOR = new Color(100, 100, 100, 50);
	public static final Color BG_COLOR = new Color(240, 225, 225);
	public static final Color LABEL_COLOR = new Color(200, 180, 180);
	public static final Color LINE_COLOR = new Color(0, 0, 0, 50);
	public static final Color RESPONSE_NAME_COLOR = new Color(0, 0, 0, 100);
	
	
	public static final int RECT_ARC_DIM = 10;

}
